  ...
  <div v-if="isOk" class="tableWap">
    ...
    <table class="shouyi">
      ...
      <tr v-for="(t, i) of shouyiArr" :key="i" :class="{ toSell: t.sell }">
        ...
        <td>
          <span
            :class="{ isGreen: !setColor(t.yingli), isRed: setColor(t.yingli) }"
            >{{ t.yingli }}</span
          >
        </td>
        <td>
...
function useHttps() {
  import("../../myself.js").then((res) => {
    let reg = /<tr>.+?<\/tr>/gi,
      ...
      reg1 = /(?<=html'>).+?(?:\（)\d{6}(?:\）)/gi, // 基金名字和代码
      reg2 = /(.+)(?:\（)(\d{6})(?:\）)/gi, // 基金代码
      reg3 = /(?<=desc'>).+?(?=(<\/td|<br\/))/gi, // 持仓
      reg4 = /(?<=f16'>)-?(\d+?,?)+?(\.\d+)(?=<\/span)/gi,
      reg5 = /(?<=f12'>)-?\d+?(\.)?\d+?%/gi; // 收益率
    shouyiArr.value = arr
      .map((t) => {
    ...
    let excelCodeArr = metaData.value.map((s) => s[0]);
    shouyiArr.value.forEach((t) => {
      if (!excelCodeArr.includes(t.code)) {
        t.sell = true;
      }
    });
  });
...
.toSell {
  background-color: rgb(155, 242, 252);
}
</style>